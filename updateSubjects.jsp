...
<%
    ...
    String message = ""; // <-- To store popup message
    ...
    try {
        ...
        if (request.getParameter("update") != null) {
            ...
            PreparedStatement updateStmt = conn.prepareStatement(
                "UPDATE subjects SET subject_name=?, sem=?, year=?, dept=?, fid=? WHERE subject_id=?"
            );
            updateStmt.setString(1, subName);
            updateStmt.setString(2, sem);
            updateStmt.setString(3, year);
            updateStmt.setString(4, dept);
            updateStmt.setString(5, fid);
            updateStmt.setString(6, subId);
        ...
        // Load all faculty
        facStmt = conn.prepareStatement("SELECT faculty_id, faculty_name, faculty_department FROM faculty");
        ...
        // Load subjects of HOD's department
...
<h2 style="text-align:center;">Update Subjects - Department: <%= hodDept.toUpperCase() %></h2>
<table>
    <tr>
        <th>Subject ID</th>
        <th>Subject Name</th>
...
%>
    <form method="post">
        <tr>
            <td><input type="text" name="subject_id" value="<%= subjectId %>" readonly></td>
            ...
            <td><input type="text" name="dept" value="<%= dept %>" readonly></td>
            <td>
                <select name="faculty">
