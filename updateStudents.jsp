...
<%
  ...
  if (request.getParameter("update") != null) {
    ...
    PreparedStatement ups = conn.prepareStatement(
      "UPDATE studentdetails SET student_name=?, stu_year=?, stu_sem=?, stu_batchyear=? WHERE student_id=?"
    );
    ups.setString(1, sname);
    ups.setString(2, syear);
    ups.setString(3, ssem);
    ups.setString(4, sbatch);
    ups.setString(5, sid);
    ups.executeUpdate();
%>
    <p class="message">✅ Updated student ID: <%= sid %></p>
<%
  }
...
%>
      <tr>
        <form method="post">
          <td><input type="text" name="sid" value="<%= rs.getString("student_id") %>" readonly></td>
          ...
          <td><%= rs.getString("student_department") %></td>
          <td>
